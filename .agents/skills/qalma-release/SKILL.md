...
3. Run `pnpm release <version>`. This is `nx release <version> --skip-publish`,
   ...
   - Commits the manifest change as `chore(release): publish <version>`.
...
4. `git push --follow-tags` to push the commit and the tag. Pushing the tag is
   the actual deploy trigger — `.github/workflows/release.yml` runs and
   publishes `@qalma/editor@<version>` with the `alpha` dist-tag.
