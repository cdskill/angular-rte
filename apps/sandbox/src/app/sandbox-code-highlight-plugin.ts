...
import { SANDBOX_CODE_BLOCK_LANGUAGES } from './sandbox-code-block';
...
function createCodeHighlightPlugin(codeBlock: NodeType): ProseMirrorPlugin {
  return new ProseMirrorPlugin<DecorationSet>({
    ...
    props: {
      ...
      nodeViews: {
        codeBlock: (node) => new SandboxCodeBlockView(node),
      },
    },
  });
}
...
class SandboxCodeBlockView implements NodeView {
  ...
  constructor(node: ProseMirrorNode) {
    ...
    this.dom.className = 'sandbox-code-block';

    const toolbar = document.createElement('div');
    toolbar.className = 'sandbox-code-block-toolbar';
    toolbar.contentEditable = 'false';

    this.languageLabel = document.createElement('span');
    this.languageLabel.className = 'sandbox-code-block-language';

    this.copyButton = document.createElement('button');
    this.copyButton.className = 'sandbox-code-block-copy';
    this.copyButton.type = 'button';
    this.copyIcon = document.createElement('span');
    this.copyIcon.className = 'sandbox-code-block-copy-icon';
    this.copyLabel = document.createElement('span');
    this.copyLabel.className = 'sandbox-code-block-copy-label';
...
function getLanguageLabel(language: string): string {
  return (
    SANDBOX_CODE_BLOCK_LANGUAGES.find((entry) => entry.value === language)
      ?.label ?? language
  );
}

type HighlightNode = HighlightElement | HighlightText;
...
function collectHighlightRange(
  node: unknown,
  start: number,
): HighlightRange {
